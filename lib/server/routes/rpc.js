...
module.exports = function(snap, server) {
  ...
  function interceptor(spark, message) {
    ...
    console.log('Message from ' + user.id + '(' + spark.id + ')', message);
  ...
  function dispatcher(spark, message) {
    ...
    var noRecipients = message.getRecipients().length === 0;

    console.log('Message to:', message.getRecipients(), 'Current:', user.id, message);

    // If  message is broadcast message or has correct recipient
    if((noRecipients && message.getSender() !== user.id) || message.hasRecipient(user.id)) {
